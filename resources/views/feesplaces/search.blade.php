...
@endif
                    ...
                    <table id="companies-data" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                        ...
                        <tbody>
                             ...
                             @foreach($feesplaces_search as $feesplace)
                            <tr role="row" class="odd">
                                <td class="sorting_1">
                                 @foreach($cities  as $city)
                                   @if($city->id == $feesplace->departure_city)
                                     {{$city->name_ru}}
                                     @endif
                                  @endforeach
                                   </td>
                                <td>
                                   @foreach($cities  as $city)
                                   @if($city->id == $feesplace->arrival_city)
                                     {{$city->name_ru}}
                                     @endif
                                  @endforeach</td>
                                ...
                                <td>@foreach($airlines as $airline)
                                  ...
                                  @endforeach
                                <td>
                                    <a href="{{ URL::to('feesplace-edit/'.$feesplace_id) }}">
                                        <span class="glyphicon glyphicon-wrench" title="Редактировать сбор">Редактировать сбор</span>
                                    </a>
                                </td>
                                    <td>
                                    <a href="{{ URL::to('feesplace-copy/'.$feesplace_id) }}">
                                        <span class="glyphicon glyphicon-wrench" title="Копировать сбор">Копировать сбор</span>
                                    </a>
                                </td>
